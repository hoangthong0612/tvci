...
@section('content')
    ...
    @endif
    <form action="{{ route('admin.posts.update', $post->id) }}" class="row" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="form-group">
                ...
                <select class="js-example-basic-single form-control" name="categories[]" multiple="multiple"
                    ...
                    @foreach ($categoryOptions as $id => $title)
                        <option value="{{ $id }}"
                            {{ in_array($id, $post->categories()->pluck('id')->toArray()) ? 'selected' : '' }}>
                            {{ $title }}
                        </option>
                ...
                <select class="js-example-basic-single form-control" id="tags" name="tags[]" multiple="multiple">
                    @foreach ($tags as $tag)
                        <option value="{{ $tag->title }}"
                            {{ in_array($tag->title, $post->tags->pluck('title')->toArray()) ? 'selected' : '' }}>
                            {{ $tag->title }}
                        </option>
                    @endforeach
                </select>
